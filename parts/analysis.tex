...
\begin{userReq}{CA}{errores}{pc=h,pd=h,s=c,v=l}
    El sistema debe tener buenos mensajes de error.
\end{userReq}

\begin{userReq}{CA}{bigints}{pc=h,pd=m,s=nc,v=h}
...
\begin{softwareReq}{FN}{compilar-instrucciones}{pc=h,pd=h,s=nc,v=h}
    {CA-funcionalidades}
    El sistema debe permitir \glsdisp{compilation}{compilar} \glspl{instruction}.
\end{softwareReq}

\begin{softwareReq}{FN}{compilar-pseudo}{pc=h,pd=h,s=nc,v=h}
    {CA-funcionalidades}
    El sistema debe permitir \glsdisp{compilation}{compilar} \glspl{pseudo-instruction}.
\end{softwareReq}

\begin{softwareReq}{FN}{directivas}{pc=h,pd=h,s=nc,v=h}
    {CA-funcionalidades}
...
\begin{softwareReq}{FN}{secciones}{pc=h,pd=h,s=nc,v=h}
    {CA-funcionalidades}
    El sistema debe soportar los siguientes tipos de secciones: datos e
    instrucciones.
\end{softwareReq}

\begin{softwareReq}{FN}{directivas-datos}{pc=h,pd=h,s=nc,v=h}
    {CA-funcionalidades}
...
\begin{softwareReq}{FN}{floats}{pc=h,pd=h,s=nc,v=h}
    {CA-funcionalidades}
...
\begin{softwareReq}{FN}{ISA}{pc=h,pd=h,s=nc,v=h}
    {CA-ISA}
    El sistema debe permitir especificar la \gls{ISA} a utilizar.
\end{softwareReq}

\begin{softwareReq}{FN}{comentarios}{pc=m,pd=l,s=nc,v=h}
    {CA-comentarios}
    El sistema debe permitir especificar la cadena utilizada como prefijo de los
    comentarios de línea.
\end{softwareReq}

\begin{softwareReq}{FN}{num-etiquetas}{pc=m,pd=l,s=nc,v=h}
    {CA-num-etiquetas}
    El sistema debe permitir utilizar cualquier cantidad de etiquetas en una
    \gls{sentence}.
\end{softwareReq}

\begin{softwareReq}{FN}{referencias}{pc=h,pd=h,s=c,v=m}
    {CA-referencias}
    El sistema debe permitir referenciar etiquetas definidas más adelante en el
    código \glsdisp{assembly}{ensamblador}.
\end{softwareReq}

\begin{softwareReq}{FN}{constantes}{pc=m,pd=m,s=nc,v=h}
...
\begin{softwareReq}{FN}{expr-operadores}{pc=m,pd=m,s=c,v=h}
    {CA-expresiones}
...
\begin{softwareReq}{FN}{expr-directiva}{pc=m,pd=m,s=nc,v=h}
    {CA-expresiones}
...
\begin{softwareReq}{FN}{mult-defs-instruccion}{pc=h,pd=m,s=nc,v=h}
...
\begin{softwareReq}{FN}{errores-base}{pc=h,pd=h,s=c,v=m}
    {CA-errores}
...
%\begin{softwareReq}{FN}{errores-extra}{pc=l,pd=l,s=c,v=h}
%    {CA-errores}
...
\begin{softwareReq}{FN}{bigints}{pc=h,pd=m,s=nc,v=h}
    {CA-bigints}
    El sistema debe permitir utilizar números enteros de cualquier tamaño para
    las direcciones de \gls{memory} y valores de las \glspl{expression}.
\end{softwareReq}
...
\begin{softwareReq}{NF}{ISA}{pc=h,pd=h,s=nc,v=h}
    {RE-ISA}
    El formato de definición de la \gls{ISA} debe ser el utilizado
    actualmente en CREATOR.
\end{softwareReq}

\begin{softwareReq}{NF}{def-instrucciones}{pc=h,pd=h,s=nc,v=h}
    {RE-ISA}
    La definición de la sintaxis de las instrucciones debe estar contenida en la
    \gls{ISA}.
\end{softwareReq}

\begin{softwareReq}{NF}{API}{pc=h,pd=h,s=nc,v=h}
    {RE-API}
    La \gls{API} de entrada y salida del sistema debe ser la utilizada
    actualmente en CREATOR.
\end{softwareReq}

\begin{softwareReq}{NF}{chrome}{pc=h,pd=m,s=nc,v=h}
    {RE-navegadores}
    El sistema se tiene que poder ejecutar en Google Chrome 70+
\end{softwareReq}

\begin{softwareReq}{NF}{firefox}{pc=h,pd=m,s=nc,v=h}
    {RE-navegadores}
    El sistema se tiene que poder ejecutar en Mozilla Firefox 60+
\end{softwareReq}

\begin{softwareReq}{NF}{safari}{pc=h,pd=m,s=nc,v=h}
    {RE-navegadores}
    El sistema se tiene que poder ejecutar en Safari 10+
\end{softwareReq}

\begin{softwareReq}{NF}{nodejs}{pc=m,pd=m,s=nc,v=h}
    {RE-nodejs}
    El sistema se tiene que poder ejecutar en Node.js
\end{softwareReq}

\begin{softwareReq}{NF}{JS}{pc=h,pd=m,s=nc,v=h}
    {RE-JS}
    El sistema debe tener una \gls{API} para \gls{JS}.
\end{softwareReq}

\begin{softwareReq}{NF}{licencia}{pc=h,pd=m,s=nc,v=h}
    {RE-FOSS}
    El sistema tiene que utilizar una licencia \gls{FOSS}.
\end{softwareReq}

\begin{softwareReq}{NF}{codigo-publico}{pc=h,pd=m,s=nc,v=h}
    {RE-FOSS}
    El código fuente del sistema tiene ser público.
\end{softwareReq}

\begin{softwareReq}{NF}{sintaxis}{pc=h,pd=h,s=c,v=m}
    {RE-sintaxis}
    El sistema tiene que utilizar la sintaxis del \gls{assembler} GNU as para
    los \glspl{program} \glsdisp{compilation}{compilados}.
\end{softwareReq}

\begin{softwareReq}{NF}{velocidad}{pc=m,pd=m,s=vi,v=l}
    {RE-velocidad}
    El sistema tiene que ser rápido. % TODO: más explicación
\end{softwareReq}
