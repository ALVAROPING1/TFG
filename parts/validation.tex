...
\begin{testCase}{VET}{API}
    {El código del sistema y del \gls{compiler} actual está descargado} % Precondición
...
\begin{testCase}{VAT}{ISA}
    {El sistema está instalado.} % Precondición
    {Todos los \glspl{program} evaluados obtienen el mismo resultado que con el \gls{compiler} actual.} % Postcondición
...
\newcommand{\evalres}{
    ...
    \begin{figure}[H]\ContinuedFloat
        ...
        {\begin{subfloatrow}
            \ffigbox[\FBwidth]
            {\caption{Compilador actual}\label{fig:eval-old-\arabic{eval}}}
...
generados con los mensajes de error del \gls{compiler} actual y de \gls{GAS}
\parencite{GNUas}, ya que es uno de los \gls{assembler} más utilizados. Para
realizar esto, se realizarán varias pruebas con \glspl{program} a los que se ha
...
alternativas con un \gls{program} complejo.
...
\gls{compiler} actual (\evalfigref{old}) produce un error incorrecto al
...
El \gls{compiler} actual (\evalfigref{old}) produce un error incorrecto al
interpretar erróneamente el código. Cabe destacar que \gls{GAS}
(\evalfigref{as}) no produce ningún error en este caso, pero un análisis del
...
\gls{compiler} actual (como \glspl{expression} aritméticas y uso de etiquetas
como valores de \glspl{data directive}). Debido a esto, para evaluar el
rendimiento en el \gls{compiler} actual, se ha tenido que modificar el
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        \legend{Compilador actual,Compilador nuevo,\gls{GAS}}
    \end{axis}
\end{tikzpicture}
